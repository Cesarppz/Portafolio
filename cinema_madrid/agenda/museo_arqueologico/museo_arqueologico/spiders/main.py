...
class Webscrape(scrapy.Spider):
    ...
    start_urls = ['http://www.man.es/man/exposicion/exposiciones-temporales/exposiciones-temporales-presentacion-2021.html']
    

    def parse(self, response, **kwargs):
        links = response.xpath('//div[@class="cte"]/h3//a/@href').getall()
    ...
    def new_parse(self, response, **kwargs):
        ...
        dirt_schedule = response.xpath('//div[@class="info"]/p[@id="fecha"]/text()').get().split('.')
        schedule = dirt_schedule[0].replace('-',' - ')
        horario = ' / '.join(response.xpath('//div[@class="cte"][position()<=3]/p[position()<=3]/following-sibling::ul/li/text()').getall())
        description =  response.xpath('//div[@class="cte"]/p[@class="ta-justify"][position()<=3]/text()').getall()
        description = ' '.join(description)
        image = 'http://www.man.es{}'.format(response.xpath('//div[@class="cim ancho "]/img/@src').get())
        ...
        yield tools.get_headers(from_date, to_date, title, category, image_name, horario, link, description, place_name='Museo Arqueológico',longitud='40.423.553',latitud='-3.689.402')
